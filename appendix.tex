...
program-and-read of an image into the array and the analog MVM scatter. The
in-memory-parallelism stills are shown earlier with the full-stack overview
(Figs. \ref{fig:parallel} and \ref{fig:circuit} in Section \ref{sec:device}).
...
\item \texttt{keshari\_phase\_energy.py}: calibration to the published
Ag\textsubscript{2}O device, the MNIST validation, the operating-envelope phase
diagrams, and the energy model. \texttt{aihwkit\_baseline.py} (run in an AIHWKit
virtual environment): trains the MNIST MLP and produces the AIHWKit analog-inference
baseline and the shared weights.
