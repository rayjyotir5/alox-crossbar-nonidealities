...
\begin{abstract}
Analog memristor crossbars compute matrix-vector products in place, but their
accuracy is governed by device and circuit non-idealities. We build one
physically grounded oxide-memristor model, calibrate it to a published
self-rectifying Ag\textsubscript{2}O device, and scale it without changing any
equation from a single cell to a 64x64 array and trained networks. The three
dominant non-idealities separate by the remedy that helps: device variability is
correctable at write time by program-verify, wire IR drop is a static distortion
that hardware-in-the-loop training largely absorbs (a spiral network recovers
from 63.5 to 92.4 percent, where output calibration alone reaches 69.3), and
thermally activated retention is only deferrable by rewriting. We chart
two-dimensional operating envelopes over array size, wire resistance, and
temperature, and validate them out-of-sample: a cheap MVM-error feature predicts
the real task accuracy of a vision-based robotics steering network, mapped
inference-only with layer tiling, a 1T1R access transistor, and ADC/DAC
quantization, with $R^2 = 0.96$. The individual phenomena are known; the
contribution is a calibrated, baselined, reproducible framework whose envelopes
act as predictive design rules. The code is released.
\end{abstract}
...
\item An inference-only mapping of a vision-based robotics steering network with
full system realism (layer tiling, a 1T1R access transistor, ADC/DAC
quantization, read noise), whose real-task-accuracy operating envelope is
validated out-of-sample ($R^2=0.96$), turning the envelope from a descriptive map
into a predictive design rule (Section \ref{sec:robotics}).
\end{enumerate}
...
matrix in one electrical step (Section \ref{sec:array}). Figures
\ref{fig:parallel} and \ref{fig:circuit} make this last point, the heart of the
neuromorphic compute stack, concrete: a digital engine performs the
matrix-vector product as $N\times M$ sequential multiply-accumulates with a
memory fetch each cycle, whereas the crossbar drives all inputs as voltages at
once and reads the whole product in a single step. The rest of the paper
...
\caption{The same matrix-vector product done two ways, shown at three times (left
to right). The digital engine steps through $N\times M$ multiply-accumulates with
a memory fetch each cycle (its counter climbs 1, mid, $N\times M$); the crossbar
finishes the whole product in one parallel read at step 1 and then holds the
answer.}
\label{fig:parallel}
\end{figure*}
...
\textbf{Calibration to the published measured curves.} Rather than assume device
parameters, we fit them to the figures in the supplementary material of Keshari
et al. \cite{keshari2026}. From their multilevel data we take the conductance
window of 3.6 to 22.3 $\mu$S and the 100 distinct conductance states they report
(about 6.6 bits per cell, 9855 signed values with a differential pair). From the
LRS retention failure plot (their Fig. S5b) we digitize the Arrhenius points
(roughly $4\times10^3$ s at 373 K and their stated ten-year retention at 298 K)
and fit an activation energy $E_a = 1.45$ eV; this reproduces their ten-year
room-temperature retention and implies retention of only hours at 85 degrees,
markedly shorter than the illustrative $E_a=1.0$ eV used in Section
\ref{sec:retention}. Their measured potentiation curves (Fig. S8) are nearly
linear (nonlinearity $\alpha_p \approx 0.02$), consistent with the
gradient-proportional update we use, and their cell distributions (Fig. S4) show
the low variation we model. We map an MLP onto this device, following their setup
(8x8-resized MNIST, 64 inputs), with 100-level quantization and programming
noise. Table \ref{tab:mnist} compares accuracy: the device penalty in our
simulator is $0.8$ points (software $94.3$ to $93.5 \pm 0.3$), close to the
$0.6$-point penalty of an established hardware-aware toolkit (AIHWKit
\cite{rasch2023}, $93.7 \pm 0.1$) and to the near-zero penalty Keshari et al.
report (96.1 to 96.08). Our absolute software baseline is below theirs, a
training and architecture difference, but the quantity the calibration validates,
the small loss the device itself adds, agrees across all three.

\begin{table}[t]
\caption{8x8 MNIST MLP on the calibrated Ag\textsubscript{2}O device; the
validated quantity is the device penalty (software minus device).}
\label{tab:mnist}
\centering
\begin{tabular}{lcc}
\toprule
Method & Accuracy (\%) & Device penalty \\
\midrule
Software (float, ours) & 94.30 & -- \\
AIHWKit analog inference \cite{rasch2023} & $93.7 \pm 0.1$ & 0.6 \\
Our calibrated simulator & $93.5 \pm 0.3$ & 0.8 \\
Keshari et al. 2026, measured \cite{keshari2026} & 96.08 & 0.02 \\
\bottomrule
\end{tabular}
\end{table}
...
room temperature to hours at 85 degrees (steep because the measured $E_a=1.45$ eV
makes retention strongly temperature-dependent). Together the two envelopes are a
...
\section{Inference-Only Mapping of a Robotics Network}
\label{sec:robotics}
The preceding envelopes use small classifiers. To test whether the same
calibrated device model constrains a realistic workload, we mapped a PilotNet
steering convolutional network onto the Ag\textsubscript{2}O crossbar in an
inference-only setting. PilotNet is a regression network that predicts a
steering angle from a forward camera frame; we use real task accuracy, defined
as the fraction of held-out frames whose predicted angle falls within a fixed
tolerance of the reference, as the figure of merit throughout. In software
(float) the network reaches $99.9$ percent on this metric, which sets the
ceiling for every analog mapping below. Figure \ref{fig:rdemo} demonstrates the
mapped inference directly: at a good operating point the crossbar reproduces the
commanded steering and stays inside the lane-keeping tolerance across a drive,
while at a degraded operating point the same network drifts out of lane.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/robotics_demo.png}
\caption{The steering network running inference on the crossbar. Top: five road
frames, each annotated with the ground-truth steering (white) and the
crossbar-commanded steering at a good operating point (teal, $N=32$, $r_w=0.5$)
and a degraded one (red, $N=128$, $r_w=4$). Bottom: the commanded steering across
a drive. At the good point the crossbar tracks the reference inside the
lane-keeping tolerance band (100 percent of frames in tolerance); at the degraded
point it drifts out (24 percent), which would steer the vehicle out of its lane.}
\label{fig:rdemo}
\end{figure*}

\subsection{Mapping with system realism}
Each convolutional and fully connected layer is lowered to a matrix product by
im2col and tiled across one or more physical arrays of size $N$. Every tile is
evaluated through the full pipeline rather than an idealized matrix multiply: a
DAC drives the rows, the column currents pass through ADC quantization, and
peripheral read noise is added at the converters. The conductances are the
calibrated Ag\textsubscript{2}O values, and the column currents are obtained
from the same nodal IR-drop solve used in the rest of this work, parameterized
by the wire-resistance ratio $r_w$. We evaluate two cell options: a passive
one-resistor (1R) cell, and a one-transistor one-resistor (1T1R) cell with an
access transistor. We sweep $40$ configurations in total: a $32$-point grid over
array size and wire resistance, $4$ ADC/DAC bit-depth settings, and $4$ ideal
references.

\subsection{Operating envelope on real task accuracy}
Figure \ref{fig:renv} shows the envelope. Real task accuracy collapses
monotonically as array size $N$ and wire resistance $r_w$ grow: for the passive
1R cell at $r_w=2$ it falls from $99.3$ percent at $N=32$ to $66.7$, $49.3$, and
$34.7$ percent at $N=64$, $96$, and $128$. Steering regression tolerates far less
analog error than the digit and spiral classifiers studied earlier, so the
largest 1R tile within two points of the float ceiling at $r_w=2$ is only $N=32$;
a demanding regression workload narrows the usable envelope sharply. The 1T1R
cell is consistently more robust at large arrays ($72.0$, $56.0$, $37.3$ percent
at $N=64$, $96$, $128$, versus $66.7$, $49.3$, $34.7$ for 1R), because the access
transistor limits per-cell current and so reduces the IR drop. The gain is modest
because the same series resistance compresses the device's already narrow
conductance window; this is the expected access-device trade-off, made
quantitative.

\subsection{ADC/DAC ablation}
Figure \ref{fig:radc} isolates converter precision. Holding $N=96$, $r_w=2$, and
the 1R cell fixed, real task accuracy is essentially flat (about $48$ to $55$
percent) from $4$-bit to full precision. In this IR-dominated regime the analog
conductance and wire-resistance non-idealities, not converter bit depth, set
accuracy. Spending bits on higher-resolution converters does not move the
operating point; relieving IR drop (smaller tiles, or the 1T1R cell) does.

\subsection{Out-of-sample validation}
An envelope is only useful if it predicts configurations not used to build it. We
test whether the cheap, network-independent MVM-error feature, a single small
random-tile solve that needs no network run, predicts the real task accuracy of
the full mapped network. Fitting a degree-2 curve of accuracy against
$\log_{10}(\text{MVM error})$ on a random $60$ percent of configurations and
predicting the held-out $40$ percent gives a held-out $R^2 = 0.96$ and a mean
absolute error of $3.9$ accuracy points (Pearson $-0.92$). The prediction holds
both within the 1R family and pooled across 1R and 1T1R, with the 1T1R points
sitting slightly above the curve by the small IR-relief margin
(Fig. \ref{fig:rval}). The feature therefore turns the envelope from a
descriptive map into a predictive design rule: a new operating point can be
placed on the envelope, within a few accuracy points, without running the
network. That is what makes the envelope a tool rather than a chart, and it is
the central result of this section.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/robotics_envelope.png}
\caption{Real task accuracy envelope for the PilotNet steering network mapped
inference-only onto the calibrated Ag\textsubscript{2}O crossbar. Accuracy is
shown over array size $N$ and wire resistance $r_w$ for the passive 1R cell and
the 1T1R cell. Both collapse monotonically with $N$ and $r_w$; the 1T1R cell is
consistently more robust at large arrays because its access transistor relieves
the IR drop. The black contour marks the float ceiling minus two points.}
\label{fig:renv}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/robotics_validation.png}
\caption{Out-of-sample validation of the cheap MVM-error feature against real
task accuracy. (a) Real accuracy versus the feature for all grid configurations,
with the degree-2 fit. (b) Held-out predictions versus actual accuracy lie on the
diagonal: $R^2=0.96$, MAE $3.9$ points, so the feature predicts the real task
accuracy of configurations it never saw, making the envelope predictive.}
\label{fig:rval}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/robotics_adcdac.png}
\caption{ADC/DAC ablation at $N=96$, $r_w=2$, 1R cell. Real task accuracy is
flat near $48$ to $55$ percent across $4$, $6$, $8$, and full-precision
converters, indicating that analog conductance and wire-resistance
non-idealities, not converter bit depth, set accuracy in this IR-dominated
regime.}
\label{fig:radc}
\end{figure*}
...
calibrate by \emph{digitizing the published measured figures} of the device (the
Arrhenius retention plot for $E_a$, the multilevel and cell-distribution plots,
the potentiation curves); the authors release no raw per-point data files, so the
fit carries figure-reading error, and the energy model is a first-order estimate
(read power plus a per-column ADC) without full peripheral, 1T1R, or interconnect
...
retention-time distributions, fitting to raw per-point data beyond the digitized
figures, and larger networks.
