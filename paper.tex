...
degrades a trained network to chance over time at elevated temperature and
...
only deferrable by rewriting. We fit the model to the measured figures in the device's supplementary material
(its conductance window, 100 states, and an activation energy $E_a=1.45$ eV from
its Arrhenius retention plot); on its 8x8 MNIST task the calibrated device adds
only a 0.8-point accuracy penalty, matching an established hardware-aware toolkit
(AIHWKit, 0.6 points) and the near-zero penalty the authors report. We then chart
...
Finally we map a vision-based robotics steering network onto the array with full
system realism (layer tiling, a 1T1R access transistor, ADC/DAC quantization, and
read noise) and show that the cheap MVM-error feature predicts its real task
accuracy on held-out configurations ($R^2=0.96$), turning the envelope into a
predictive design rule.
