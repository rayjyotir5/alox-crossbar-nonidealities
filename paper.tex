...
\title{From Device Physics to In-Situ Learning:\\
A Unified Simulation Study of AlO\textsubscript{x} Memristor\\
...
single, physically grounded model of an aluminium-oxide (AlO\textsubscript{x})
valence-change cell and scale it, without changing any equation, from one device
up to a 64x64 crossbar and a small trained network. Using one consistent model
...
only deferrable by rewriting. The individual phenomena are each established in
prior work; our contribution is a single consistent model and a unified,
reproducible, seed-averaged comparison that makes their different characters
explicit. All experiments run in minutes on a laptop CPU and the code is
...
we report a convolutional-layer fidelity rather than a full detector, and do not
claim silicon-accurate energy or accuracy numbers, nor do we fit the model to
measured AlO\textsubscript{x} data. Importantly, none of the three phenomena is
itself novel; each is documented in the literature cited in
Section \ref{sec:relwork}. The contribution is the unified, seed-averaged,
baselined account on one consistent model, which we regard as reaching a
reproducible-study or workshop bar rather than a new-mechanism result; closing
the gap to a main-conference contribution would require measured-device
validation, larger networks, and comparison against established hardware-aware
training toolkits.

\section{Conclusion}
Starting from one physically grounded AlO\textsubscript{x} device equation and
scaling it without modification to a 64x64 array and a trained network, we gave a
unified, seed-averaged account of three crossbar non-idealities. Under this model
they tend to separate by remedy: variability is corrected at write time
($88.3$ to $96.5$ percent), IR drop is absorbed by hardware-in-the-loop training
($63.5$ to $92.4$ percent, where output calibration alone reaches only $69.3$),
and retention is only deferrable. None of these phenomena is individually new; the
value is the controlled, reproducible comparison in one place. Future work
includes a 1T1R access-device model, measured or finite-difference in-situ
gradients, broader retention-time distributions, fitting the model to measured
AlO\textsubscript{x} data, and comparison against hardware-aware training
toolkits on larger tasks.
